...
| `imports`       | Allowed dependency references; each identity may appear once. |
...
`apiVersion` and revision IDs never appear in references. Within one `imports` field, `(kind, name)` may appear only once.
