...
└── console/
    ...
    └── src/
        ...
        │   ├── expression/              # FreeMarker compilation and evaluation foundation
        │   ├── resource/                # revisions, validation, snapshots and runtime acquisition
        │   ├── policy/                  # API authorization and object filtering RFC
        │   ├── workflow/                # durable execution RFC
...
| Expression compilation          | `expression/internal/compilation/FreeMarkerExpressionCompiler.java`              | `ExpressionContract`, function registry, and template-model factory.    |
...
Taskmigo uses Apache FreeMarker as its Java Expression engine. A field contract supplies the expected result type and a profile such as `GENERAL`, `WORKFLOW`, or `POLICY_PREDICATE`; the profile controls the visible data model, Taskmigo functions, directives, and built-ins.

Keep the engine in a small foundation module so Resource, Policy, and Workflow depend on one implementation:
...
└── internal/
    ├── compilation/{FreeMarkerExpressionCompiler,TypedExpressionCompiler}.java
    ├── evaluation/{CompiledFreeMarkerExpression,TaskmigoTemplateModelFactory}.java
    ├── function/ExpressionFunctionRegistry.java
    └── configuration/FreeMarkerConfigurationFactory.java
```

Pin FreeMarker in `console/build.gradle.kts`; do not inherit an incidental transitive version:
...
| Type / function                                | Responsibility and behavior                                                                                                     |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `FreeMarkerConfigurationFactory.create()`      | Creates one long-lived, immutable `Configuration` pinned to the selected FreeMarker compatibility version.                      |
| `ExpressionCompiler.compile(source, contract)` | Selects typed-expression or rendered-template compilation, validates the profile, and returns an immutable compiled form.       |
| `ExpressionContract`                           | Declares the profile, context schema, expected result type, output limit, and whether literal text or directives are permitted. |
| `TaskmigoTemplateModelFactory.create(context)` | Converts approved scalars, maps, and lists into the exact FreeMarker data model exposed by the contract.                        |
| `ExpressionFunctionRegistry`                   | Registers explicit `TemplateMethodModelEx` or directive implementations by profile; duplicate names fail startup.               |
| `CompiledExpression.evaluate(context)`         | Evaluates with bounded output/iteration limits and returns the typed result or a source-aware diagnostic.                       |
| `ExpressionDiagnostic`                         | Carries a stable code, manifest path, line/column, and safe message without context values or secrets.                          |

Configure FreeMarker with a fixed compatibility version, `SimpleObjectWrapper`, `TemplateExceptionHandler.RETHROW_HANDLER`, and `api_builtin_enabled = false`. Do not expose Java beans, reflection, static models, class resolution, or a template loader. Manifest Expressions cannot include or import files. Construct the data model from immutable maps, lists, strings, numbers, booleans, and Taskmigo-owned `TemplateModel` implementations only.

```java
Configuration configuration = new Configuration(Configuration.VERSION_2_3_34);
configuration.setDefaultEncoding(StandardCharsets.UTF_8.name());
configuration.setObjectWrapper(new SimpleObjectWrapper(Configuration.VERSION_2_3_34));
...
Create and publish this `Configuration` once, then never mutate it. Its template loader remains unset, so `#include` and `#import` cannot resolve external content. Compile manifest-owned source directly from an in-memory `Reader` and cache the immutable result by Expression source, contract/profile version, and expected result type.

An entire YAML value containing one interpolation, such as `'${step.output.status == "done"}'`, is a typed Expression. Validate the interpolation boundary, compile its inner FreeMarker expression into an internal assignment, process it, then read the resulting `TemplateModel` from the `Environment` and convert it to the field's expected type. Do not infer typed values by parsing rendered text.

A string or node field may instead compile a normal FreeMarker template containing literal text, interpolation, and permitted directives. Render through a size-limited `Writer`; enforce contract limits on source length, nesting, sequence size, and iteration. The same source and context must produce deterministic output.

Policy uses the restricted `POLICY_PREDICATE` profile. Request mode evaluates a boolean through the FreeMarker engine. Object mode parses the documented FreeMarker predicate subset into a typed database-neutral AST; it does not depend on FreeMarker internal AST classes and never renders SQL. Maintain shared conformance fixtures so request evaluation and object compilation agree on operators, built-ins, missing values, and types. The public syntax remains canonical in [Expressions](/versions/v0/manuals/expressions) and [Policy predicates](/versions/v0/manuals/manifests/v0-policy#available-predicates).
