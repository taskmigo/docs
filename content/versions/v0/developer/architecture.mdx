...
| Framework mapping    | Configuration/persistence/web adapter           | Keep Spring, Nimbus, JPA, and FreeMarker types out of core domain contracts.          |

If two capabilities need a common primitive, first decide whether one capability should own and expose it. Create a shared foundation only for truly technical, stable code such as the `expression` engine, correlation IDs, or a generic outbox primitive, and protect the allowed contents with architecture tests. Shared code must not contain business endpoints, domain repositories, or feature-specific DTOs. Taskmigo functions such as `Trans` remain owned by their business capability and register through the public Expression SPI.
...
3. FreeMarker Expression foundation, profiles, Taskmigo functions, and security/conformance tests.
4. Versioned Manifest contracts for Site, Application, Page, Component, and Translation.
5. Resource graph resolution and diagnostics.
6. Snapshot compilation, persistence, atomic activation, and runtime acquisition.
7. Page/Application runtime using only compiled snapshots.
8. Role/Group domain and effective-role resolution in `access`.
9. Policy predicate parser, snapshot contributor, request authorization, then object-query adapter.
10. Workflow definition compiler and persistence state machine.
11. Workflow worker, outbox, Actions, UI interaction, and operational recovery.
