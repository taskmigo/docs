...
Treat revision identity and resolved graph identity as separate verification boundaries. The Resource/runtime suite must prove all of these together:

- changing only a child Resource creates a new child revision but does not create new revisions for unchanged importers;
- the changed child's graph checksum propagates through every affected ancestor to the root Site graph checksum;
- an unaffected branch keeps both its revision ID and graph checksum;
- the same exact revision selections and dependency relationships produce the same graph checksums regardless of database row or traversal order;
- a changed root Site graph can produce and activate a new snapshot while `siteRevisionId` remains unchanged;
- the activated snapshot pins the exact revision ID and graph checksum for every compiled Resource selection.

Use a fixture such as `Site → Application → Page → Component`, change only the Component, and assert both sides of the invariant: ancestor revision IDs stay stable while Page/Application/Site graph checksums change.
...
| Candidate does not activate              | Candidate correlation ID, exact revision selections, root graph checksum, resolution/validation diagnostics, compiler capability, activation version conflict. |
