...
<Callout title='Start with product behavior' type='info'>
  Manuals are the source of truth for user-visible semantics. [Product status](/versions/v0/manuals/status) separates
  defined contracts from RFCs and planned delivery. Developer pages must not turn an RFC or open decision into current
  behavior.
</Callout>
