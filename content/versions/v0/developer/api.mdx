...
Resource identity remains `(kind, metadata.name)`. `apiVersion` selects the revision contract and is not part of identity.

HTTP paths include both contract version and kind name so transport routing does not require an encoded slash. A manifest with:
...
The URL segments `v0/page` identify the requested contract/kind combination; they do not redefine Resource identity as `(v0/page, home)`.
...
Use `PUT /api/v0/resources/{kindVersion}/{kindName}/{name}` with the complete YAML source.
