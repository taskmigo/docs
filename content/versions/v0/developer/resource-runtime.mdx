...
<Callout title='Implementation target' type='info'>
  A browser application instance keeps one **runtime binding** across client-side navigation. The binding keeps the same
  snapshot and presentation context, while security and business state can still be evaluated using their own freshness
  rules.
</Callout>
...
<Callout title='Never reacquire current runtime during navigation' type='warning'>
  Do not call `ActiveSiteSnapshotProvider.acquire()` without the bound snapshot identity after binding validation. Doing so
  can mix an old layout with a newly activated Page graph.
</Callout>
...
Parse YAML and retain source locations.
...
Validate `apiVersion`, `kind`, metadata, kind-local fields, and Expression syntax/result types.
...
Parse references, select exact revisions, and validate reference-ability and cycles.
...
Validate cross-resource rules, compiler capabilities, and snapshot contributors.
...
Calculate graph checksums bottom-up and compile runtime artifacts.
