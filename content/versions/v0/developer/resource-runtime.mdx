...
<Callout title='Never reacquire the active snapshot during navigation' type='warning'>
  After validation, every route, Page, Component, Translation, and Expression lookup must use the bound snapshot. Silently
  switching to the newly active snapshot can create a UI that never existed as one valid version.
</Callout>
