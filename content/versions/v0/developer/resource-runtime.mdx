...
The `resource` capability owns authoring resources, immutable revisions, tags, validation, dependency resolution, snapshot compilation, diagnostics, atomic activation, and the snapshot side of runtime binding. Do not distribute these responsibilities across the `system` module.
...
├── package-info.java
├── ActiveSiteSnapshotProvider.java       # public snapshot acquisition port
├── RuntimeBindingService.java             # bootstrap/validate app-instance binding
...
public interface RuntimeBindingService {
  RuntimeBinding bootstrap(PresentationDefaults defaults, RendererCompatibility renderer);
  ValidatedRuntimeBinding validate(OpaqueRuntimeBinding token, RendererCompatibility renderer);
}
```

`RuntimeBinding` is a logical contract, not a requirement for a database row. It binds one browser application instance to an immutable snapshot and captured presentation context such as locale. The public/browser representation must be opaque; callers must not construct a binding from raw snapshot UUIDs.
...
| `ActiveSiteSnapshotProvider.acquire()` | Return an immutable handle to the snapshot active at acquisition time. |
| `RuntimeBindingService.bootstrap(...)` | Acquire the active snapshot once and capture presentation defaults for a new browser application instance. |
| `RuntimeBindingService.validate(...)` | Recover/validate the same snapshot and presentation context for later navigation; never reacquire current defaults as a substitute. |

## Browser application consistency

The consistency boundary is longer than one HTTP request. A browser application instance normally keeps one runtime binding from its initial full document load until its next full reload.
...
sequenceDiagram
  ...
  Next->>Profile: read presentation defaults
  Next->>Runtime: bootstrap(defaults, renderer)
  Runtime-->>Next: binding B1 = snapshot S42 + locale ja-JP
  Next-->>Browser: SSR + opaque B1

  Browser->>Next: client navigation + B1
  Next->>Runtime: render(path, B1)
  Runtime-->>Next: model from S42 + ja-JP

  Note over Profile: another tab may change profile locale

  Browser->>Next: later navigation + B1
  Next->>Runtime: render(path, B1)
  Runtime-->>Next: still S42 + ja-JP
```

Do not implement runtime consistency by repeatedly reading `site_runtime_state.active_snapshot_id`. That pointer is consulted during bootstrap; later navigation uses the binding's snapshot. Likewise, mutable user-profile locale is a bootstrap default, not the locale source for every render.

An explicit presentation change in the same tab establishes a replacement binding/rerender for that tab. Do not mutate bindings belonging to other tabs. A full reload bootstraps again and can therefore adopt both a newer snapshot and newer profile defaults.

The binding must be able to become invalid because of expiry, emergency revocation, or renderer/artifact incompatibility. Exact token representation and retention policy are unresolved; keep the service boundary independent of whether the implementation becomes a signed stateless token, a server-side lease, or a hybrid.

Presentation pinning must not accidentally pin security authority. Locale, future timezone, or SSR theme are presentation candidates; authorization, account status, permission revocation, and business data require the freshness rules of their owning capabilities.
...
Activation affects new bindings. Existing valid bindings continue to reference their immutable snapshot until normal reload or a future expiry/revocation rule requires replacement.
...
| `site_runtime_state` | singleton PK/check; `site_name`; `active_snapshot_id`; `version`; `activated_at` | The one atomically replaceable pointer used to bootstrap new runtime bindings. |

No `runtime_binding` table is required by the current design. Do not add one until the binding representation and retention/revocation strategy are decided. A stateless signed binding may need no per-tab durable row at all.

Use foreign keys for ownership and deletion safety. Revisions and snapshots referenced by `site_snapshot_resource`, active runtime state, Workflow execution, or any future durable binding/retention mechanism cannot be physically deleted. Cleanup operates only on provably unreachable candidates and snapshots.
...
- a runtime binding remains on its snapshot after newer activation;
- a bound locale remains stable after another tab changes the profile default;
- full reload can bootstrap a newer snapshot and current presentation defaults;
