...
<Callout title='RFC — not implemented' type='warning'>
  This contract may change before adoption. Check [Product status](/versions/v0/manuals/status) before relying on it.
</Callout>
...
Workflow fields marked as Expression-capable use [Expressions](/versions/v0/manuals/expressions) with these additional values:
...
navigation:
  - when: '${step.output.status == "approved"}'
    goto: approved
  - when: '${step.output.status == "needs-review"}'
    goto: review
  - goto: rejected
```
...
Starting an execution pins its Site snapshot, Workflow revision, and imported dependencies. Later changes affect new executions only. See [Runtime lifecycle](/versions/v0/manuals/runtime#workflow-execution).
...
A Page imports Workflows that its users may discover and start. Workflow-to-Workflow imports are not supported. See [Reference ability](/versions/v0/manuals/resources#reference-ability).
