...
Policy statements use the regular Go `text/template` `{{ ... }}` action syntax, but permit only the functions below. Request mode evaluates them against request context. Object mode compiles them to the equivalent parameterized PostgreSQL operation.

| Function     | Arguments            | Object-mode PostgreSQL equivalent                              |
| ------------ | -------------------- | -------------------------------------------------------------- | --- | ---------------------------- |
...
| `in`         | `value candidate...` | `value = ANY(:candidates)`                                     |
| `notIn`      | `value candidate...` | `value <> ALL(:candidates)`                                    |
| `contains`   | `collection value`   | `value = ANY(:collection)` or the registered column equivalent |
| `startsWith` | `value prefix`       | `value LIKE escape(:prefix)                                    |     | '%' ESCAPE '\\'`             |
| `endsWith`   | `value suffix`       | `value LIKE '%'                                                |     | escape(:suffix) ESCAPE '\\'` |
...
Every dynamic operand becomes a bound parameter. The shown names are explanatory rather than literal SQL placeholders. `contains` may target a Principal/Request collection parameter or a Resource collection with a registered database mapping; snapshot validation rejects it when no equivalent exists.
