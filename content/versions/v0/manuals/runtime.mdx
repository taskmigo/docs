...
```mermaid
flowchart LR
  request["Next.js SSR request"] --> acquire["Acquire active snapshot"]
  acquire --> route["Resolve Application + Page route"]
  route --> render["Load compiled Page + Components"]
  render --> expression["Evaluate Expressions in backend"]
  expression --> model["Return resolved render model"]
  model --> next["React / Next.js SSR"]
```

Next.js receives resolved values in the render model. It does not receive Expression source or an Expression program that the browser must execute. Frontend-only Expression context is outside the v0 contract.
