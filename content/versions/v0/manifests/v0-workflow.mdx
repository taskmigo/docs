...
<TypeTable
  type={{
    ...
    'spec.inputs': {
      ...
      typeDescriptionLink: '#input-definitions',
      required: false,
      description: 'Values accepted when an execution starts.',
    },
    ...
    'spec.steps': {
      ...
      typeDescriptionLink: '#step-kinds',
      required: true,
      description: 'Defines executable steps by stable identifier.',
    },
    ...
    'StepDefinition.timeout': {
      type: 'duration',
      description: 'Limits one Action attempt.',
    },
    'StepDefinition.retry': {
      type: 'object',
      description: 'Configures retry behavior for an Action step.',
    },
    'StepDefinition.retry.attempts': {
      type: 'integer',
      description: 'Limits the total number of Action attempts.',
    },
    'StepDefinition.retry.delay': {
      type: 'duration',
      description: 'Waits this long between Action attempts.',
    },
    ...
    'StepDefinition.ui': {
      type: 'object',
      description:
        'Waits for interaction rendered by an imported Component; mutually exclusive with `action` and `fail`.',
    },
    'StepDefinition.ui.component': {
      type: 'string',
      description: 'Selects an imported Component by name.',
    },
    'StepDefinition.ui.props': {
      type: 'Dictionary<unknown>',
    ...
    'StepDefinition.fail.message': {
      type: 'string',
      description: 'Records the failure message.',
    },
    'StepDefinition.navigation': {
      type: 'object[]',
      description: 'Tests routes in order and selects the first matching destination.',
    },
    'StepDefinition.navigation[].when': {
      type: <FieldType supportsExpression={true}>boolean</FieldType>,
      description: 'Controls whether this navigation route matches; omission defines a fallback.',
    },
    'StepDefinition.navigation[].goto': {
      type: 'string',
      required: true,
...
`Dictionary<InputDefinition>` means each user-defined input name maps to the `type`, `required`, and optional `default` fields below. `Dictionary<StepDefinition>` means each stable step identifier maps to exactly one Action, Interactive UI, or Fail structure documented under [Step kinds](#step-kinds).
